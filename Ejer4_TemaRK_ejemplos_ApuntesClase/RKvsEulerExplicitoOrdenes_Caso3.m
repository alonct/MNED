...
ytrue=3*exp(ttrue.^2/2)-ttrue.^2-2;
...
for n=1:N
    %Calculo RK
    k1=mifun3(t(n),yRK(n));
    k2=mifun3(t(n)+h,yRK(n)+h*k1);
    k3=mifun3(t(n)+h,yRK(n)+h/2*(k2+k1));
    yRK(n+1)=yRK(n)+h*(3*k1+k2+2*k3)/6;
    %Calculo Euler
    yEuler(n+1)=yEuler(n)+h*mifun3(t(n),yEuler(n));
end 
figure(1);
...
plot(log(nP),log(errEuler),'*',log(nP),-log(nP),'-',...
    log(nP),log(errRK),'bo',log(nP),-2*log(nP)+2,'g-');
